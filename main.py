...
def gather_personal_data():  # from relative path assets/private
    GMAIL_USERNAME = (
        open(absolute_path("assets/personal/gmail_username.txt"), "r")
        .readline()
        .strip()
    )
    GMAIL_API_KEY = (
        open(absolute_path("assets/personal/gmail_app_password.txt"), "r")
        .readline()
        .strip()
    )
...
if (
    input(f"Send {len(gather_subscriber_data('subscribers.csv'))} emails? (Y/N)\n")
    == "Y"
):
    send()
else:
